...
Two filters, `rank` and `played`, appear at the top of `advanced filters` only when you're logged in with osu!supporter; osu! ignores them without a token. The other osu!-website facets, `explicit` in the filters block plus `genre`, `language`, `extra` at the top of `advanced filters`, work without supporter and always render.
...
All six facets force the osu! api route. Losing supporter status resets `rank` and `played` to their defaults.
...
Under that comes the set's full difficulty list, one line per diff: the name, then its star rating and a ten-cell meter reading one cell per star. Ratings line up in a column past the longest name; a name too long for the pane is cut short rather than pushing its rating off. <kbd>j</kbd> / <kbd>k</kbd> move through the list when the preview has focus. The marked diff's own AR / CS / OD / HP, bpm, length, object counts, and pass rate fill the block below. On a narrow pane the meters drop and the ratings stay.
...
On a terminal with graphics support (sixel, kitty, or iterm2, konsole included) the highlighted set's cover art renders against the preview's right edge, with the metadata to its left; past the bottom of the image the metadata spans the full pane width. A roomy pane and a short title get a wide crop; a longer title shrinks the cover to a smaller square so the title still fits on one line, and only a title too long for even that wraps to two lines. Elsewhere it falls back to unicode half-blocks; a cramped preview drops the image. Covers load in the background a moment after a row settles under the cursor. A cover deep enough to reach the difficulty list holds the list back until past the image, so the list gets the whole pane width instead of the strip beside the art. On a pane too short to fit it down there the list stays put and takes the strip. The cover only renders on the preview's top row: scroll down and the text takes the whole pane, scroll back up and the cover returns.
