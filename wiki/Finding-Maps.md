...
On a terminal with graphics support (sixel, kitty, or iterm2, konsole included) the highlighted set's cover art renders beside the metadata, against the preview's right edge; elsewhere it falls back to unicode half-blocks. A narrow or short preview drops the cover and keeps the text. Covers load in the background a moment after a row settles under the cursor.
