...
pub(crate) async fn download_pass(
    ...
    if beatmapset_ids.is_empty() {
        debug!(download_id = id, "download pass invoked with no targets");
    ...
    let mut beatmap_queue: VecDeque<u32> = beatmapset_ids.into_iter().collect();
    let mut retried_integrity: HashSet<u32> = HashSet::new();
    let mut refreshed_sets: HashSet<u32> = HashSet::new();
    let concurrency = thread_count.max(1);
    ...
    while !beatmap_queue.is_empty() || !join_set.is_empty() {
        while join_set.len() < concurrency {
            ...
            let mirror_pool_for_task = mirror_pool.clone();
            ...
            let retry_phase_for_task = retry_phase;
        ...
        let (slot, beatmapset_id, result) = match task_result {
            ...
            Err(err) => {
                aborted = true;
                warn!(download_id = id, error = %err, "Download task panicked");
                break;
            }
        };
        ...
        match result {
            Ok(DownloadResult::Success(file)) => {
                mirror_pool.clear_penalty(file.mirror);
                let file_path = output_dir.join(file.filename.as_ref());
                let _ = tx.send(DownloadEvent::ThreadStatus {
                    id,
                    thread_index: slot,
                ...
                .await
                {
                    Ok(()) => {
                        ...
                        verified.insert(beatmapset_id);
                        if let Some(remaining) = outstanding.remove(beatmapset_id).await {
                            let _ = tx.send(DownloadEvent::DownloadTarget { id, remaining });
                        }
                    ...
                    Err(outcome) => {
                        ...
                        if already_retried {
                            if let ArchiveOutcome::Invalid {
                                beatmapset_id: Some(set_id),
                                ..
                            } = &outcome
                            {
                                if refreshed_sets.insert(*set_id) {
            ...
            Ok(DownloadResult::Skipped(filename)) => {
                ...
                debug!(download_id = id, beatmapset_id, "Skipped beatmap download");
                let _ = tx.send(DownloadEvent::BeatmapStatus {
                    id,
                    beatmapset_id,
            ...
            Ok(DownloadResult::Aborted) => {
                ...
                warn!(download_id = id, beatmapset_id, "Download aborted mid-pass");
                let _ = tx.send(DownloadEvent::BeatmapStatus {
                    id,
                    beatmapset_id,
            ...
            Err(err) => {
                ...
                warn!(download_id = id, beatmapset_id, error = %message, "Download errored");
                let _ = tx.send(DownloadEvent::BeatmapStatus {
                    id,
                    beatmapset_id,
                    stage: BeatmapStage::Failed,
    ...
    if aborted {
        warn!(download_id = id, "Download pass aborted; exiting loop");
    }
